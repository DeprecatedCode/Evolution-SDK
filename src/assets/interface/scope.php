	...
	public function get($var_map) {
		...
		if(substr($map[0], 0, 1) == ':') {
			...
			switch($map[0]) {
				...
				break;
				case ':controller':
					$source = e::controller($map[1]);
					$flag_first = 2;
				break;
				case ':upgrade':
					$source = e::upgrade($map[1]);
					$flag_first = 2;
				break;
				case ':session':
				...
				case ':profile':
					...
					if(is_numeric($id) && $id > 0) {
						$source = e::app('members')->account($id);
					} else {
						$source = e::$session->member;
					}
					$flag_first = 1;
				...
				break;
				case ':generate':
					$source = new IXML_Generator;
					$flag_first = 1;
				break;
				case ':cache':
					$source = cache::get($map['1'],$map['2']);
					$flag_first = 3;
				break;
				case ':key':
					return $this->pointer;
				...
				break;
				case ':benchmarks':
					$source = array('exec_time' =>round((microtime(true)-START_TIME)*1000,1), 'query_count' => Database_MySQL::$query_count, 'query_time' => round(Database_MySQL::$qt,1), 'parse_time' => round(Interface_Parser::$parse_time,1));
					$flag_first = 1;
				break;
				...
				break;
				case ':protocol':
					$source = empty($_SERVER['https']) ? 'http' : 'https';
				break;
				case ':iterator':
					if(isset($this->data[':forkey'])){
						$source = array('key' => $this->data[':forkey'], 'index' => $this->data[':forkey'] + 1, 'odd' => $this->data[':forkey'] & 1, 'first' => $this->data[':forkey'] == 0);
					}else{
		...
		if(!$flag_first) {
			...
			if(is_string($map[0])) {
				...
				e::fault(100, 'ixml_scope_no_func_call');
			}
		}
		...
		if($filters)
			foreach($filters as $filter) {
				if(is_array($filter)) {
					e::filter($source, $filter['func'], $filter['args']);
				} else {
					e::filter($source, $filter, array());
				}
			}
...
class IXML_Generator {
	public function options() {
		$x = func_get_args();
		$o = '';
		$selected = array_shift($x);
		foreach($x as $k) {
			$k = explode(':', $k, 2);
			list($value, $name) = $k;
			$value = trim($value);
			$name = trim($name);
			$o .= '<option value="'.$value.'"'.($value == $selected ? ' selected="selected">' : '>').$name.'</option>';
		}
		return $o;
	}
	
	public function __toString() {
		return 'IXML Generator';
	}
