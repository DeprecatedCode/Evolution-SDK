...
class Interface_ParserNew {
	...
	function __construct($string, $html_object = false, $al = '') {
		if(isset(self::$cache[$string]))
			...
			if(!file_exists(ROOT_INTERFACE.'/'.$string))
				e::fault(100, 'ixml_interface_not_exist', array('interface' => $string));
			$this->html = file_get_contents(ROOT_INTERFACE.'/'.$string);	
	...
	function parse() {
		...
		$this->object = $this->object ? $this->object : new InterfaceBuilder;
	...
	function get_tag($force_html) {
		...
		if(strlen($text_node) != 0) {
			$text_vars = $this->extract_vars($text_node);
			$this->pointer = $s;
