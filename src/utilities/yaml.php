...
/**
 * Utility for reading YAML files.
 *
 * @package evolution
 * @author David Boskovic
 **/
class e_utility_yaml
{
	
	public function string($string) {
		return e_Spyc::YAMLLoadString($string);
	}
	public function file($file) {
		return e_Spyc::YAMLLoad($file);
	}
} // END class e_utility_yaml
...
class e_Spyc {
	...
	private function _dumpNode($key, $value, $indent, $previous_key = -1, $first_key = 0) {
		...
		if (is_string($value) && ((strpos($value, "\n") !== false || strpos($value, ": ") !== false || strpos($value, "- ") !== false || strpos($value, "*") !== false || strpos($value, "#") !== false || strpos($value, "<") !== false || strpos($value, ">") !== false || strpos($value, "[") !== false || strpos($value, "]") !== false || strpos($value, "{") !== false || strpos($value, "}") !== false) || substr($value, -1, 1) == ':')) {
