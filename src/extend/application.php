...
class App {
	...
	public function __construct($owner = false) {
		...
		if($this->_application) $this->configure = new e_Configure(ROOT_APPLICATIONS.'/'.$this->_application.'/configure');
		if($this->_application) $this->documentation = new e_Documentation(ROOT_APPLICATIONS.'/'.$this->_application.'/documentation');
	...
	protected function _list($page, $page_length, $sort_by, $table, $map, $module) {
			...
			if(!$this->owner) {
				...
				if(!class_exists($module)) {				
					e::fault(100, 'application_module_not_exist',array('application_name' => $this->_application, 'map' => $map));
				}
				while($page = $pages->row()) {
					$result[] = new $module($page);
				}
	...
	public function _query_list($table = false, $map = false, $page = false, $page_length = false, $custom_query = false, $sort_by = false) {
		...
		if(!$table) e::fault(100, 'application_query_list_no_table',array('application_name' => $this->_application, 'map' => $map));
		
		# fault if no map is passed
		if(!$map) e::fault(100, 'application_query_list_no_map',array('application_name' => $this->_application, 'map' => $map));
		...
		if(!class_exists($module)) {				
			return e::fault(100, 'application_module_not_exist',array('application_name' => $this->_application, 'map' => $map));
		}
		while($page = $pages->row()) {
			$result[] = new $module($page);
		}
	...
	public function __call($method, $args) {
		if(isset($this->_lists[$method])) {
			...
			$table = str_replace('.','_',$map);
			$module = 'App_'.$table;
			return $this->_list($args[0], $args[1], $args[2], $table, $map, $module);
		}
		elseif(isset($this->_items[$method])) {
			$map = $this->_items[$method];
			$table = str_replace('.','_',$map);
			$module = 'App_'.$table;
			if(!class_exists($module)) {				
				return e::fault(100, 'application_module_not_exist',array('application_name' => $this->_application, 'map' => $map));
			...
			return e::fault(100, 'application_method_not_exist',array('application_name' => $this->_application, 'method' => $method));
