...
# include all assets
include(ROOT_FRAMEWORK.'/assets/cache.php');
...
class e
{
	...
	public static function helper($name, $val = NULL, $val2 = NULL, $val3 = NULL, $val4 = NULL) {
		...
		if(isset(self::$cache['helper'][$name]))
			return self::$cache['helper'][$name];
		
		# if the file hasn't been included yet (faster than include_once)
		if(!isset(self::$cache['included_helpers'][$name])) {
			...
			self::$cache['included_helpers'][$name] = true;
		}
	...
	public static function hook($hook_name, $obj, $args) {
		if(isset(self::$cache['hooks'][$hook_name])) {
			$func = self::$cache['hooks'][$hook_name];
	...
	public static function create() {
		if(!isset(self::$cache['create']))
			self::$cache['create'] = new Utility_Create();
	...
	public static function db($dbtype='mysql') {
		if(!isset(self::$database_engines[$dbtype]))
			die("You're trying to intialize a database engine ($dbtype) that hasn't been loaded.");
		else
	...
	private static function _get_cache($library, $key) {
		return self::_check_cache($library, $key) ? self::$cache[$library][$key] : false;
	}
